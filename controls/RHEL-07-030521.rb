...
# impacts
title 'RHEL-07-030521 - All uses of the su command must be audited.'
control 'RHEL-07-030521' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030521'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030521_chk'
  tag fixid: 'F-RHEL-07-030521_fix'
  tag version: 'RHEL-07-030521'
  tag ruleid: 'RHEL-07-030521_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the “su” command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “su” command occur.
...
# START_DESCRIBE RHEL-07-030521
  describe command('auditctl -l') do
    its('stdout') { should match /^-a always,exit -F path=\/bin\/su -F perm=x -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 -k privileged-priv_change/ }
  end
# STOP_DESCRIBE RHEL-07-030521
