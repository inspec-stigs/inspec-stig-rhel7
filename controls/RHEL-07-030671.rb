...
# impacts
title 'RHEL-07-030671 - All uses of the delete_module command must be audited.'
control 'RHEL-07-030671' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030671'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030671_chk'
  tag fixid: 'F-RHEL-07-030671_fix'
  tag version: 'RHEL-07-030671'
  tag ruleid: 'RHEL-07-030671_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the delete_module command occur. 
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the delete_module command occur. 
...
# START_DESCRIBE RHEL-07-030671
  describe auditd_rules.syscall('delete_module').arch('b32').key('module-change').action('always').list do
    it { should eq(['exit']) }
  end

  if os[:arch] == 'x86_64'
    describe auditd_rules.syscall('delete_module').arch('b64').key('module-change').action('always').list do
      it { should eq(['exit']) }
    end
  end
