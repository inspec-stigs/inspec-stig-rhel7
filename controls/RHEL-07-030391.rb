...
# impacts
title 'RHEL-07-030391 - All uses of the fchmod command must be audited.'
control 'RHEL-07-030391' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030391'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030391_chk'
  tag fixid: 'F-RHEL-07-030391_fix'
  tag version: 'RHEL-07-030391'
  tag ruleid: 'RHEL-07-030391_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the “fchmod” command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “fchmod” command occur.
...
# START_DESCRIBE RHEL-07-030391
  describe command('auditctl -l') do
    its('stdout') { should match /^-a always,exit -F arch=b32 -S fchmod -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 key=perm_mod/ }
    its('stdout') { should match /^-a always,exit -F arch=b64 -S fchmod -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 key=perm_mod/ }
  end
# STOP_DESCRIBE RHEL-07-030391
