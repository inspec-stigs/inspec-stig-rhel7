...
# impacts
title 'RHEL-07-030753 - All uses of the unlink command must be audited.'
control 'RHEL-07-030753' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030753'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030753_chk'
  tag fixid: 'F-RHEL-07-030753_fix'
  tag version: 'RHEL-07-030753'
  tag ruleid: 'RHEL-07-030753_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the unlink command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the unlink command occur.
...
# START_DESCRIBE RHEL-07-030753
  describe auditd_rules.syscall('unlink').arch('b32').perm('x').key('delete').action('always').list do
    it { should eq(['exit']) }
  end

  if os[:arch] == 'x86_64'
    describe auditd_rules.syscall('unlink').arch('b64').perm('x').key('delete').action('always').list do
      it { should eq(['exit']) }
    end
  end
