...
# impacts
title 'RHEL-07-030751 - All uses of the renameat command must be audited.'
control 'RHEL-07-030751' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030751'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030751_chk'
  tag fixid: 'F-RHEL-07-030751_fix'
  tag version: 'RHEL-07-030751'
  tag ruleid: 'RHEL-07-030751_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the renameat  command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the renameat command occur.
...
# START_DESCRIBE RHEL-07-030751
  describe auditd_rules.syscall('renameat').arch('b32').perm('x').key('delete').action('always').list do
    it { should eq(['exit']) }
  end

  if os[:arch] == 'x86_64'
    describe auditd_rules.syscall('renameat').arch('b64').perm('x').key('delete').action('always').list do
      it { should eq(['exit']) }
    end
  end
