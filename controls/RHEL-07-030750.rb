...
# START_DESCRIBE RHEL-07-030750
  describe auditd_rules.syscall('rename').arch('b32').perm('x').key('delete').action('always').list do
    it { should eq(['exit']) }
  end

  if os[:arch] == 'x86_64'
    describe auditd_rules.syscall('rename').arch('b64').perm('x').key('delete').action('always').list do
      it { should eq(['exit']) }
    end
  end
