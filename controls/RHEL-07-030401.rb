...
# impacts
title 'RHEL-07-030401 - All uses of the fsetxattr command must be audited.'
control 'RHEL-07-030401' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030401'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030401_chk'
  tag fixid: 'F-RHEL-07-030401_fix'
  tag version: 'RHEL-07-030401'
  tag ruleid: 'RHEL-07-030401_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the “fsetxattr” command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “fsetxattr” command occur.
...
# START_DESCRIBE RHEL-07-030401
  describe command('auditctl -l') do
    its('stdout') { should match /^-a always,exit -F arch=b32 -S fsetxattr -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 key=perm_mod/ }
    its('stdout') { should match /^-a always,exit -F arch=b64 -S fsetxattr -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 key=perm_mod/ }
  end
# STOP_DESCRIBE RHEL-07-030401
