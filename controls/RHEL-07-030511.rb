...
# impacts
title 'RHEL-07-030511 - All uses of the unix_chkpwd command must be audited.'
control 'RHEL-07-030511' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030511'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030511_chk'
  tag fixid: 'F-RHEL-07-030511_fix'
  tag version: 'RHEL-07-030511'
  tag ruleid: 'RHEL-07-030511_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the “unix_chkpwd” command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “unix_chkpwd” command occur.
...
# START_DESCRIBE RHEL-07-030511
  describe command('auditctl -l') do
    its('stdout') { should match /^-a always,exit -F path=\/sbin\/unix_chkpwd -F perm=x -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 -k privileged-passwd/ }
  end
# STOP_DESCRIBE RHEL-07-030511
