...
# START_DESCRIBE RHEL-07-030425
  describe auditd_rules.syscall('ftruncate').arch('b32').exit('-EPERM').key('access').action('always').list do
    it { should eq(['exit']) }
  end

  if os[:arch] == 'x86_64'
    describe auditd_rules.syscall('ftruncate').arch('b64').exit('-EACCES').key('access').action('always').list do
      it { should eq(['exit']) }
    end
  end
