...
# impacts
title 'RHEL-07-030525 - All uses of the chsh command must be audited.'
control 'RHEL-07-030525' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030525'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030525_chk'
  tag fixid: 'F-RHEL-07-030525_fix'
  tag version: 'RHEL-07-030525'
  tag ruleid: 'RHEL-07-030525_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the “chsh” command occur.
...
-a always,exit -F path=/usr/bin/chsh -F perm=x -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 -k privileged-priv_change'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “chsh” command occur.
...
# START_DESCRIBE RHEL-07-030525
  describe command('auditctl -l') do
    its('stdout') { should match /^-a always,exit -F path=\/usr\/bin\/chsh -F perm=x -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 -k privileged-priv_change/ }
  end
# STOP_DESCRIBE RHEL-07-030525
