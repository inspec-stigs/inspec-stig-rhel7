...
# impacts
title 'RHEL-07-030402 - All uses of the lsetxattr command must be audited.'
control 'RHEL-07-030402' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030402'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030402_chk'
  tag fixid: 'F-RHEL-07-030402_fix'
  tag version: 'RHEL-07-030402'
  tag ruleid: 'RHEL-07-030402_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the “lsetxattr” command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “lsetxattr” command occur.
...
# START_DESCRIBE RHEL-07-030402
  describe auditd_rules.syscall('lsetxattr').arch('b32').key('perm_mod').action('always').list do
    it { should eq(['exit']) }
  end

  if os[:arch] == 'x86_64'
    describe auditd_rules.syscall('lsetxattr').arch('b64').key('perm_mod').action('always').list do
      it { should eq(['exit']) }
    end
  end
# STOP_DESCRIBE RHEL-07-030402
