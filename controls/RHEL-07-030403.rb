...
# START_DESCRIBE RHEL-07-030403
  describe auditd_rules.syscall('removexattr').arch('b32').key('perm_mod').action('always').list do
    it { should eq(['exit']) }
  end

  if os[:arch] == 'x86_64'
    describe auditd_rules.syscall('removexattr').arch('b64').key('perm_mod').action('always').list do
      it { should eq(['exit']) }
    end
  end
