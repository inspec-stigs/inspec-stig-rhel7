...
# impacts
title 'RHEL-07-030424 - All uses of the truncate command must be audited.'
control 'RHEL-07-030424' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030424'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030424_chk'
  tag fixid: 'F-RHEL-07-030424_fix'
  tag version: 'RHEL-07-030424'
  tag ruleid: 'RHEL-07-030424_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the truncate command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “truncate” command occur.
...
# START_DESCRIBE RHEL-07-030424
  describe command('auditctl -l') do
    its('stdout') { should match /^-a always,exit -F arch=b32 -S truncate -F exit=-EPERM -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 -F key=access/ }
    its('stdout') { should match /^-a always,exit -F arch=b64 -S truncate -F exit=-EACCES -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 -F key=access/ }
  end
