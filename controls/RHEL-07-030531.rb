...
# impacts
title 'RHEL-07-030531 - All uses of the umount command must be audited.'
control 'RHEL-07-030531' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030531'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030531_chk'
  tag fixid: 'F-RHEL-07-030531_fix'
  tag version: 'RHEL-07-030531'
  tag ruleid: 'RHEL-07-030531_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the umount command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “umount” command occur.
...
# START_DESCRIBE RHEL-07-030531
  describe auditd_rules.syscall('all').path('/bin/umount').perm('x').key('privileged-mount').action('always').list do
    it { should eq(['exit']) }
  end
