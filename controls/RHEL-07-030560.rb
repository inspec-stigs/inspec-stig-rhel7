...
# impacts
title 'RHEL-07-030560 - All uses of the pt_chown command must be audited.'
control 'RHEL-07-030560' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030560'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030560_chk'
  tag fixid: 'F-RHEL-07-030560_fix'
  tag version: 'RHEL-07-030560'
  tag ruleid: 'RHEL-07-030560_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the pt_chown command occur. 
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the pt_chown command occur. 
...
# START_DESCRIBE RHEL-07-030560
  describe command('auditctl -l') do
    its('stdout') { should match /^-a always,exit -F path=\/usr\/libexec\/pt_chown -F perm=x -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 -k privileged_terminal/ }
  end
# STOP_DESCRIBE RHEL-07-030560
