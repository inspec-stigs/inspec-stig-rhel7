...
# impacts
title 'RHEL-07-030383 - All uses of the fchownat command must be audited.'
control 'RHEL-07-030383' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030383'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030383_chk'
  tag fixid: 'F-RHEL-07-030383_fix'
  tag version: 'RHEL-07-030383'
  tag ruleid: 'RHEL-07-030383_rule'
  tag fixtext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “fchownat” command occur.
...
The audit daemon must be restarted for the changes to take effect.'
  tag checktext: 'Verify the operating system generates audit records when successful/unsuccessful attempts to use the “fchownat” command occur.
...
# START_DESCRIBE RHEL-07-030383
  describe auditd_rules.syscall('fchownat').arch('b32').key('perm_mod').action('always').list do
    it { should eq(['exit']) }
  end

  if os[:arch] == 'x86_64'
    describe auditd_rules.syscall('fchownat').arch('b64').key('perm_mod').action('always').list do
      it { should eq(['exit']) }
    end
  end
