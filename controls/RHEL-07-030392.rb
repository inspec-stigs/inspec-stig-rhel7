...
# START_DESCRIBE RHEL-07-030392
  describe auditd_rules.syscall('fchmodat').arch('b32').key('perm_mod').action('always').list do
    it { should eq(['exit']) }
  end

  if os[:arch] == 'x86_64'
    describe auditd_rules.syscall('fchmodat').arch('b64').key('perm_mod').action('always').list do
      it { should eq(['exit']) }
    end
  end
