...
# impacts
title 'RHEL-07-030541 - All uses of the postqueue command must be audited.'
control 'RHEL-07-030541' do
  impact 0.5
  ...
  tag 'stig', 'RHEL-07-030541'
  tag severity: 'medium'
  tag checkid: 'C-RHEL-07-030541_chk'
  tag fixid: 'F-RHEL-07-030541_fix'
  tag version: 'RHEL-07-030541'
  tag ruleid: 'RHEL-07-030541_rule'
  tag fixtext: 'Configure the operating system to generate audit records when successful/unsuccessful attempts to use the postqueue command occur. 
...
# START_DESCRIBE RHEL-07-030541
  describe command('auditctl -l') do
    its('stdout') { should match /^-a always,exit -F path=\/usr\/sbin\/postqueue -F perm=x -F auid>=1000 -F auid!=4294967295 -F subj=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0\.c1023 -k privileged-postfix/ }
  end
